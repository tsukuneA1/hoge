from collections.abc import Iterator

from libs.infrastructure.db.gen import crawl_targets, models
from sqlalchemy import Connection


class CrawlTargetsRepository:
    def __init__(self, connection: Connection):
        self.querier = crawl_targets.Querier(conn=connection)

    def list(
        self, limit: int, max_attempts: int, lease_timeout: float
    ) -> Iterator[models.CrawlTarget]:
        return self.querier.list_ingest_targets(
            max_attempts=max_attempts,
            lease_timeout_seconds=lease_timeout,
            row_limit=limit,
        )

    def success(self, pkey: str) -> models.CrawlTarget | None:
        return self.querier.mark_crawl_target_succeeded(pkey=pkey)

    def fail(self, *, last_error: str | None, pkey: str) -> models.CrawlTarget | None:
        return self.querier.mark_crawl_target_failed(pkey=pkey, last_error=last_error)

    def upsert(
        self,
        pkey: str,
        last_seen_run_id: int | None,
        discovered_year: int,
        source_page: int,
    ) -> models.CrawlTarget | None:
        return self.querier.upsert_crawl_target(
            pkey=pkey,
            last_seen_run_id=last_seen_run_id,
            discovered_year=discovered_year,
            source_page=source_page,
        )
