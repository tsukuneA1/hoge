...
id は 28 文字固定の識別子であり、これを主キーとして使用する。
...
| カラム | 型 | 理由 |
|--------|-----|------|
| `id` | `CHAR(28)` | 固定長28文字のため `CHAR` が適切。UUIDより意味のある識別子 |
...
CREATE TABLE reviews (
    ...
    id            CHAR(28)        NOT NULL REFERENCES syllabuses(id) ON DELETE CASCADE,
...
```sql
-- キーワード検索（英語タイトル対象）
SELECT id, title, instructors, semester, credits
FROM syllabuses
...
-- 複合フィルタ
SELECT id, title, instructors, credits
FROM syllabuses
WHERE year = 2026
  AND semester = 'spring'
...
def upgrade() -> None:
    ...
    op.create_table(
        "syllabuses",
        sa.Column("id", sa.CHAR(28), primary_key=True),
...
```
1. 新年度クローリング実行（年1回）
2. syllabuses テーブルに UPSERT（id が同じなら updated_at を更新）
3. 前年度データはそのまま保持（year カラムで区別）
```

```sql
-- クローラーによる UPSERT
INSERT INTO syllabuses (id, title, year, semester, credits, department,
                        instructors, description, objectives, schedule,
                        evaluation, textbooks, raw_html, crawled_at)
VALUES (:id, :title, :year, :semester, :credits, :department,
        :instructors, :description, :objectives, :schedule,
