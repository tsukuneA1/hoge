...
- 主に使用するのは以下のパラメータ
    ...
    - p_page: ページネーションのページ番号指定。
- 一覧検索ではPOSTだったが詳細取得ではGETメソッドが使用可能。pKey(シラバスの主キー)をクエリパラメータとして https://www.wsl.waseda.jp/syllabus/JAA104.php?pKey=2600001002012026260000100226&pLng=jp のような形式で指定する
...
- アプリケーションログは標準出力/標準エラーに出力し、Cloud Loggingで収集する
- ログにはrun_id, job_type, page, p_key, status, retry_countなどを含める
- HTML本文やraw_htmlはログに出力しない
- ジョブ単位のサマリはcrawl_runsに保存する
- pKey単位の失敗理由はcrawl_targets.last_errorに保存する
...
```mermaid
sequenceDiagram
    participant Job as discover service
...
```mermaid
sequenceDiagram
    participant Job as ingest service
    participant Runs as crawl_runs repository
    participant Targets as crawl_targets repository
    participant HTTP as httpx client
    participant Parser as parser
    ...
    loop each page
        ...
        Courses-->>Job: inserted/updated/unchanged
...
5. stale running(statusがrunningかつ updated_at + lease_timeout が現在時刻より小さいもの) は再処理できる
